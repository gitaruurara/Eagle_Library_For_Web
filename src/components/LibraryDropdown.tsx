...
const switchLibrary = async (libraryName: string) => {
  try {
    const response = await fetch(`${API_BASE_URL}/library/switch/${libraryName}`);
    if (!response.ok) {
...
const LibraryDropdown: React.FC<LibraryDropdownProps> = ({ onLibrarySwitch }) => {
  ...
  useEffect(() => {
    const fetchLibraries = async () => {
      try {
        const libraryListResponse = await fetch(`${API_BASE_URL}/library_list`);
        ...
        const currentLibraryResponse = await fetch(`${API_BASE_URL}/library/current`);
